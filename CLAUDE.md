...
- **Stable toolchain drift before release:** `dtolnay/rust-toolchain@stable` can advance before a developer's installed `stable-aarch64-apple-darwin` toolchain. Before release preflight, run `rustup update stable-aarch64-apple-darwin` and confirm `rustc --version`; otherwise a new Clippy lint can pass locally and fail the macOS CI job.
