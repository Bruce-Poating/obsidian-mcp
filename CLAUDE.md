...
- **`serde_json::Value` tool-input schemas:** Schemars emits an unconstrained schema for `serde_json::Value`, so rmcp `tools/list` advertises no `type` for those fields. Some MCP clients then send array/object inputs as JSON-encoded strings. Object-only inputs need an object-shaped schema plus matching deserialization (or a map type) and must reject non-objects before any write; genuinely dynamic JSON inputs need an explicit schema covering their supported types. For optional dynamic values, Serde normally collapses explicit JSON `null` and a missing field into `None`; use a custom deserializer when null is a valid supplied value. Test the generated tool schema and raw JSON/MCP deserialization boundary, not only handlers called with in-process `json!` values. Do not heuristically parse every string as JSON/YAML because literal string values are valid.
...
- **CI/CD workflows:** Two GitHub Actions workflows: `ci.yml` runs `cargo fmt --check` (Ubuntu only) + `cargo clippy` + `cargo test` on a 3-OS matrix (ubuntu-latest, macos-latest, windows-latest), `cargo check --all-features` on Windows, the upgrade CLI contract for `embeddings-api`, `embeddings`, and both features together on Ubuntu, and Ubuntu daemon integration tests with embeddings. `release.yml` triggers on `v*` tags, builds 5 targets (x86_64-unknown-linux-gnu, aarch64-unknown-linux-gnu, x86_64-apple-darwin, aarch64-apple-darwin, x86_64-pc-windows-msvc), creates a GitHub Release with checksums, and publishes to crates.io.
- **GitHub Actions cache keys:** `actions/cache` rejects commas in cache keys. When a matrix value is a comma-separated Cargo feature list, keep that value for the Cargo command but define a separate delimiter-safe matrix value for cache identity.
- **Platform-specific Clippy:** Helpers reachable only from Unix implementations must be excluded from non-Unix library builds; use `#[cfg(any(unix, test))]` when their platform-independent unit tests should still compile. Windows Clippy denies otherwise-unused helpers as dead code.
- **Stable toolchain drift before release:** `dtolnay/rust-toolchain@stable` can advance before a developer's installed stable toolchain or while their default remains pinned. Before release preflight, run `rustup update stable-aarch64-apple-darwin` and confirm with `rustup run stable rustc --version`; otherwise a new Clippy lint can pass locally and fail CI.
...
- **Embedding search (Layer 2):** Opt-in via `OBSIDIAN_EMBEDDINGS=true` + either `--features embeddings` (local fastembed) or `--features embeddings-api` (OpenAI-compatible API). Both features can be enabled simultaneously. `VaultInner` holds one `EmbeddingRuntime`, which loads the backend, reconciles the cache, and reports readiness in the background. `Vault::open()` never waits for model load, inference, or persistence. Exposed as the `search_semantic` MCP tool.
...
- **fastembed model resolution:** Resolve a configured local model deterministically by fastembed enum name, exact model code, or unique repository basename. The documented `BAAI/bge-small-en-v1.5` alias resolves to canonical `BGESmallENV15`; invalid or ambiguous names are background load failures, never silent fallback to a different model. `get_model_info` requires importing `fastembed::ModelTrait`.
- **Embedding cache contract:** The binary cache keeps its existing path but now has a magic/version envelope, exact `EmbeddingSpaceIdentity`, `first_pass_complete`, normalized paths, full prepared-text SHA-256 hashes, and vectors. Decoding is bounded and rejects trailing bytes, duplicate/invalid paths, identity/dimension mismatches, and non-finite values before publication. Old payloads rebuild once in the background; incomplete checkpoints may resume privately but are never queryable.
- **Embedding-space identity boundary:** Compatibility requires backend kind, canonical resolved model identifier, vector dimension, input-pipeline version, and an API endpoint fingerprint for remote backends. The fingerprint avoids persisting a credential-bearing URL. It cannot detect a provider silently changing weights behind the same endpoint/model identifier; do not claim stronger remote revision proof.
- **Atomic embedding persistence:** `EmbeddingStore::save` validates the whole snapshot, writes a same-directory temporary file, flushes and `sync_all`s it, then atomically persists it over the destination. Readers therefore observe the complete old or new cache, never a partially overwritten destination. Directory fsync is intentionally omitted because the cache is rebuildable.
- **Managed embedding coordinator:** `EmbeddingRuntime` is the only store writer and cache persister for a vault/context. Direct writes and both watchers update the primary/Tantivy indexes first, then submit coalesced latest-per-path Upsert/Remove intents. Watcher tasks keep only `EmbeddingRuntimeWeak`, so they cannot extend coordinator lifetime past `Vault`/context drop. Bounded batches, generations, retry backoff, and a dirty-flush interval prevent unbounded detached tasks, stale commits, per-note cache rewrites, and busy-vault persistence starvation.
- **Embedding readiness and LKG:** Status is `warming`, `ready`, or `degraded`, with queryable/indexed/total/pending fields derived from current primary-index membership. Initial note paths are queued before model loading, so progress remains truthful even while the loader is blocked; later events replace those intents latest-wins. A compatible completed cache is pruned to current index membership and may serve as same-space last-known-good during refresh; failures retain the old vector and matching old hash together. A new failed note remains absent. Deleted/excluded paths are filtered again at result construction.
- **Local embedding write visibility:** Semantic updates are intentionally eventually consistent through the managed runtime. Tests that write a note and immediately query semantic search must poll bounded status/results rather than assume synchronous visibility.
- **Non-fatal embedding errors:** Model, note-read, inference, or persistence failures never roll back a successful note write or block core tools. The runtime retries note-read, inference, and persistence failures with capped backoff; a model-load/configuration failure remains degraded until restart. `embedding_load_error()` preserves that first model-load diagnostic contract.
...
- **`EmbeddingBackend` dual-dispatch enum:** `EmbeddingModel` holds an `EmbeddingBackend` enum with `Local(Box<Mutex<TextEmbedding>>)` and `Api(ApiEmbeddingClient)` variants, each `#[cfg]`-gated by individual feature. The `Local` variant is `Box`ed to satisfy `clippy::large_enum_variant`. `embed_batch()` dispatches via match. `load()` uses `resolve_provider()` which infers from `cfg!()` when no explicit provider is given (local preferred when both features are compiled).
...
- **API request executor:** `reqwest::blocking::Client` must not be constructed, called, or finally dropped on a Tokio worker. `ApiEmbeddingClient` owns two dedicated OS worker threads and a bounded two-request channel, allowing a foreground query against last-known-good state while background reconciliation is blocked without creating unbounded tasks. Its `Drop` closes the channel and joins both workers.
- **API dimension probing:** When `OBSIDIAN_EMBEDDING_DIM` is not set, background `load_api()` sends a single-text embedding request (`"dim"`) to detect the model's output dimensionality. This avoids hardcoding dimension per model without delaying MCP initialization. The probe uses the same request executor and response validation as ordinary inference.
...
- **API key zeroize:** API request workers share an `Arc<zeroize::Zeroizing<String>>`; joining the workers on `ApiEmbeddingClient` drop releases and zeroes the last key allocation. `zeroize` is a direct dep (already a transitive dep via rustls).
- **`parse_embedding_response` integrity:** The shared API parser requires exactly one vector per input. Response items must be either all indexed or all unindexed; indexed items must be unique, contiguous, and in range before reordering. Common validation then rejects wrong dimensions and non-finite values. Never pair API results with inputs through truncating `zip` semantics.
...
- **Embedding runtime shutdown:** The final runtime handle marks the coordinator dead, synchronizes with both in-memory store commits and atomic cache publication, wakes and aborts the async worker, and invalidates pending generations. A started `spawn_blocking` inference cannot be cancelled by Tokio, but its result cannot mutate the published store or cache after runtime drop returns.
...
- **Daemon vault context lifecycle:** `ensure_vault` attaches the primary/Tantivy context and background embedding runtime without waiting for semantic warm-up. Its additive v1 response reports truthful `ready`, `phase`, indexed/total/pending counts, and a safe error. `search_semantic`, `search_hybrid`, and `open_hint` still require prior attachment; attached-but-unqueryable semantic calls return structured `ERR_VAULT_NOT_READY` (`-32030`).
- **Daemon per-vault embedding cache path:** daemon-managed caches remain under `<semantic-home>/vaults/<vault_id>/embeddings.bin` (not inside `.obsidian/obsidian-mcp`). The shared `EmbeddingRuntime`, not the watcher, reconciles and persists this cache.
...
- **MCP semantic runtime modes:** `OBSIDIAN_SEMANTIC_MODE` controls MCP behavior: `daemon` is strict daemon-only, `local` forces in-process embeddings, and `auto` prefers daemon with local fallback only for genuine bootstrap/transport/unavailability failures. An attached daemon returning `ERR_VAULT_NOT_READY` is warming/degraded, not unavailable, and must not start a competing local runtime.
...
- **Legacy embedding cache migration path:** Cache relocation belongs to the managed embedding coordinator, never MCP/Vault startup. For a daemon context it prefers the active local `.obsidian-mcp/embeddings/embeddings.bin`, then the older `.obsidian/obsidian-mcp/embeddings.bin`. Relocation rejects sources above the cache-size limit before copying and also bounds the copy against concurrent source growth. It uses a same-directory temporary file plus atomic no-clobber publication, is idempotent, and never deletes the source; strict identity validation still decides whether the relocated payload can be served.
...
- **Windows health response termination:** A valid HTTP response may end with `ErrorKind::ConnectionAborted` on Windows where Unix commonly reports `ConnectionReset`. The upgrade health probe treats either as end-of-response only after receiving bytes, then still requires a complete HTTP 200 body with the exact `obsidian-mcp` identity and non-empty version.
...
- **Watcher lock discipline:** Both watchers release the `VaultIndex` write lock before Tantivy batch operations, then submit embedding intents without inference or persistence. This minimizes index contention and keeps filesystem event processing independent from semantic backend latency.
...
- **CI cargo-audit:** The CI pipeline runs `cargo audit` on Ubuntu to detect known vulnerabilities in dependencies automatically. Prefer a lockfile-only `cargo update -p <crate>@<old> --precise <patched>` when existing dependency constraints admit the patched transitive version; suppress only advisories that cannot be fixed without a major upgrade, with the risk assessment documented in `.cargo/audit.toml`. Update that file when dependencies are upgraded.
...
- **Self-upgrade identity and activation:** `obsidian-mcp upgrade` accepts only the exact Cargo-tracked official crates.io installation that owns both shipped binaries. The embedded target and exact non-default feature set must agree with Cargo's tracker and both post-install binaries. An unchanged healthy runtime is not restarted; an old or unhealthy supported runtime is reconciled. Linux process discovery must normalize the ` (deleted)` suffix from `/proc/<pid>/exe` so an old in-memory process remains discoverable after binary replacement.
- **Semantic daemon upgrade monotonicity:** Only `sibling` and `managed_download` daemons are locally owned for automatic replacement; `path`, `override`, and unresolved legacy origins remain external. Before shutdown, bind endpoint health to the live PID and verify that PID's executable against the manifest. Upgrade and normal bootstrap may activate a newer verified sibling, but must never replace a newer running daemon with an older installed sibling.
...
- **`/health` endpoint:** HTTP mode exposes `GET /health` returning `{"status":"ok","server":"obsidian-mcp","version":"...","notes":<count>}`. Embedding builds add live `embeddings_ready`, `embeddings_phase`, `embeddings_indexed_notes`, `embeddings_total_notes`, `embeddings_pending_notes`, and optional `embeddings_error`; HTTP health remains `ok` while semantic indexing warms or degrades because core service availability is independent. MCP tools are served under `/mcp`.
...
- **Tool consolidation (29→18):** 7 groups of related tools merged into parameterized tools using action/query/type/view string parameters. Consolidation map: `vault_structure`→`vault_list(format:"tree")`, `frontmatter_*`→`frontmatter(action)`, `note_metadata`+`note_document_map`→`note_inspect(view)`, `links_*`→`wikilinks(query)`, `periodic_*`→`periodic(action)`, `note_append`+`note_prepend`→`note_insert(position)`, `search_tag`+`search_frontmatter`→`search_metadata(type)`. Action/type params are `Option<String>` (not enums) for LLM-friendly schema. Validation returns `INVALID_PARAMS` for unknown values or missing required fields. The later additive `note_read_many` tool brings the current total to 19 without changing `note_read`.
- **Tool filtering (`OBSIDIAN_TOOLS`):** `ToolFilter` enum in `config.rs` with `Full`/`Profile(String)`/`AllowList(HashSet)`/`DenyList(HashSet)` variants. Detection: contains `!`→deny-list; single word matching profile→profile; single word matching tool name→1-tool allow-list; single unknown word→error; comma-separated→allow-list. `disabled_tools()` returns the complement set for `disable_route()`. Unknown tool names in lists are warned, not errored. Profiles: `full`(19), `core`(15), `read`(11), `minimal`(6). `note_read_many` is intentionally absent only from `minimal`; `ALL_TOOL_NAMES` is the single source of truth for all 19 names.
- **Bounded bulk note reads:** `note_read_many` accepts exactly one of explicit `paths` or a `dir`; explicit order is preserved and directory order comes from sorted `Vault::list_files`. Defaults are 20 inspected files and 64 KiB of returned content, with hard caps of 100 files and 256 KiB. Zero limits and selector misuse are `INVALID_PARAMS`. Stat preflight prevents reading an already-oversized file, actual UTF-8 bytes are rechecked after reading, and an oversized first note is never returned. Skip details are capped at 100 while `skipped_count` remains exact. The typed `Json<NoteReadManyOutput>` route deliberately emits both `structuredContent` and compatibility JSON text; use `note_read` for an intentionally oversized note.
- **`vault_list` metadata compatibility:** `include_metadata` defaults false so existing list-mode string arrays and tree text remain unchanged. In list mode, `true` enriches indexed case-insensitive Markdown paths from the current `NoteMetadata` snapshot with filename-stem title, tags, size, and optional timestamps; directories, non-Markdown files, and excluded/unindexed notes stay path-only. Tree mode plus metadata is `INVALID_PARAMS`. Metadata for paths already returned by `Vault::list_files` must use one exact-path batch index lookup; calling the canonicalizing `get_note_metadata` per entry repeatedly scans directories and becomes quadratic for large flat folders.
...
- **Embedding cache relocation:** Local cache location remains `{mcp_data}/embeddings/embeddings.bin`; the legacy `{vault}/.obsidian/obsidian-mcp/embeddings.bin` is best-effort relocated by the background runtime when the destination is absent and is never deleted. The current runtime rejects the legacy payload contract and rebuilds it in the background. All cache destination writes, including relocation, belong to the runtime coordinator and use atomic publication; watchers only submit intents. The daemon independently uses its context-owned cache path.
